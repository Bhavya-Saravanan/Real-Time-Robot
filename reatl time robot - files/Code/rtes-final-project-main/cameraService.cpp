 ...
 * Description: File contains function definitions related to camera capture Service
 *              Contains necessary function for camera init and camera capture
 *
...
/*
we first dequeue from buffer and check if we have data
If we do not have data and get a EAGAIN code we return early to prevent
blocking. The image is then convered to bgr format and stored in a protected global
variable
*/
void camera_capture_service()
{
    ...
    service1_ok = true;
    // syslog(LOG_INFO, "Service 1 OK set");
}
